...
export default async function AboutPage() {
  ...
  const vivianImage =
    get(content, "vivian_bio", "image") ||
    "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=1000&q=80";

  const values = [
    {
      title: get(content, "values", "value_1_title", "Obsessive Detail"),
      body: get(content, "values", "value_1_body", ""),
    },
    {
      title: get(content, "values", "value_2_title", "Deep Personalisation"),
      body: get(content, "values", "value_2_body", ""),
    },
    {
      title: get(content, "values", "value_3_title", "Flawless Execution"),
      body: get(content, "values", "value_3_body", ""),
    },
    {
      title: get(content, "values", "value_4_title", "Lasting Memories"),
      body: get(content, "values", "value_4_body", ""),
    },
  ];
  ...
  return (
    <main className="bg-ivory">
      <section className="relative flex min-h-[50vh] items-center justify-center overflow-hidden pt-20">
        ...
        <div className="relative z-10 px-[5%] text-center">
          ...
          <h1 className="font-display text-5xl italic text-white md:text-7xl">
            {get(content, "hero", "headline", "The Story Behind Vivabloom")}
          </h1>
          {get(content, "hero", "subheadline") ? (
            <p className="mx-auto mt-4 max-w-xl font-body text-white/75">
              {get(content, "hero", "subheadline")}
            </p>
          ) : null}
        </div>
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-16 px-[5%] py-24 lg:grid-cols-2">
        ...
        <div>
          ...
          <h2 className="mb-6 font-display text-4xl italic text-onyx md:text-5xl">
            {get(content, "vivian_bio", "headline", "Hi there, I'm Vivian")}
          </h2>
          <div className="space-y-6 font-body leading-[1.85] text-charcoal">
            {get(content, "vivian_bio", "body_1") ? <p>{get(content, "vivian_bio", "body_1")}</p> : null}
            {get(content, "vivian_bio", "body_2") ? <p>{get(content, "vivian_bio", "body_2")}</p> : null}
            {get(content, "vivian_bio", "body_3") ? <p>{get(content, "vivian_bio", "body_3")}</p> : null}
          </div>
          {get(content, "vivian_bio", "pullquote") ? (
            <blockquote className="my-8 border-l-2 border-champagne pl-6">
              <p className="font-display text-2xl italic leading-tight text-onyx">
                {get(content, "vivian_bio", "pullquote")}
              </p>
            </blockquote>
          ) : null}
      ...
      <section className="px-[5%] py-24">
        ...
        <h2 className="mb-16 text-center font-display text-4xl italic text-onyx md:text-5xl">
          {get(content, "values", "headline", "What We Stand For")}
        </h2>
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-10 md:grid-cols-2">
          {values.map((v) => (
            ...
            >
              ...
              {v.body ? <p className="mt-4 font-body leading-relaxed text-charcoal">{v.body}</p> : null}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-[5%] py-24 text-center">
        ...
        <h2 className="mb-8 font-display text-3xl italic text-onyx md:text-4xl">
